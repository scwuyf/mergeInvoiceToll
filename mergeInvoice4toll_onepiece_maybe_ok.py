...
def process_summarysheet(pdf_path, output_folder, summary_number, Binding_Position):
    """处理 PDF 文件"""
    ...
    doc.close()
    new_doc.close()
    print(f"已经保存了用于合并的的汇总单临时文件 {summary_number}_{summary_page_position}_票据汇总单_temp4prt.pdf")

    # 检测文件名是否包含 "1piece"
    if '1piece' in os.path.basename(pdf_path):
        # 合并具有相同summary_number且文件名以ipiece结尾的全部文件
        merged_pdf_path = merge_1piece_files(output_folder, summary_number)
        if merged_pdf_path:
            # 调整页面到A4尺寸
        ...
        print(f"文件名 {os.path.basename(pdf_path)} 不包含 1piece，不进行额外处理。")


def merge_1piece_files(output_folder, summary_number):
    # 使用 PdfWriter 替换 PdfMerger
    writer = PdfWriter()
    
    # 过滤文件列表，只保留以summary_number开头、以1piece结尾的文件
    pdf_files = [f for f in os.listdir(output_folder) 
                 if f.startswith(summary_number) and f.endswith('1piece.pdf')]
    
    if not pdf_files:
        print(f"没有找到任何以 {summary_number} 开头且以 _1piece 结尾的 PDF 文件")
        return
    ...
    for pdf_file in pdf_files:
        ...
        with fitz.open(pdf_path) as doc:
            for page in doc:
                page = pdf[0]
                # 裁剪页面到610 x 394点大小
                rect = fitz.Rect(0, 0, 610, 394)
                cropped_page = doc.new_page(width=610, height=394)
                cropped_page.show_pdf_page(rect, doc, page.number)
                cropped_pdf_writer.add_page(cropped_page)
    
    merged_pdf_path = os.path.join(output_folder, f"{summary_number}_merged_1piece_cropped.pdf")
    with open(merged_pdf_path, "wb") as fout:
        cropped_pdf_writer.write(fout)
    
    print(f"已将 {summary_number} 的 1piece 文件合并为 {merged_pdf_path}")
    return merged_pdf_path    
    
def process_files(folder_path, summary_pdf_files):
    for pdf_file in summary_pdf_files:
        pdf_path = os.path.join(folder_path, pdf_file)
        temp_folder_path = os.path.join(folder_path, 'tempfolder')
        try:
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if summary_number:
                    ...
                    for i, df in enumerate(tables):
                        invoice_data_rows= len(df)
                        print(f"表格 {i+1} 的行数: {len(df)}")

                    # 判断发票数据行数
                    if invoice_data_rows <= 4:
                        # 处理单个发票文件的情况
                    ...
                    # 处理PDF文件
                    process_summarysheet(pdf_path, output_folder, summary_number, Binding_Position)
                
        except Exception as e:
            #messagebox.showerror("错误", f"处理文件 {pdf_file} 时发生错误: {e}")
            print("错误", f"处理文件 {pdf_file} 时发生错误: {e}")
...
def match_invoices(invoice_numbers, folder_path, summary_number, output_folder):
                ...
                try:
                    with pdfplumber.open(pdf_path) as pdf:
                        ...
                        if pdf_invoice_number == invoice_number:
                            ...
                            new_file_path = os.path.join(output_folder, new_file_name)
                            shutil.copy(pdf_path, new_file_path)
                            matched_files.append(new_file_path)
                except Exception as e:
                    messagebox.showerror("错误", f"处理文件 {pdf_file} 时发生错误: {e}")
    ...
    if len(matched_files) == 1:
        single_file_path = matched_files[0]
        single_file_name = os.path.basename(single_file_path)
        new_single_file_name = f"{summary_number}_{invoice_numbers[0]}_1piece.pdf"
        new_single_file_path = os.path.join(output_folder, new_single_file_name)
        os.rename(single_file_path, new_single_file_path)
        print(end='\n')
        print(f"已经把仅有一张发票汇总单对应的发票文件更名为 {summary_number}_{invoice_numbers[0]}_1piece.pdf")
    ...
    final_pdf_path = adjust_pages_to_a4(merged_pdf_path, output_folder, summary_number, Binding_Position)  # 调整页面大小并合并到A4
...
def merge_pdfs(summary_number, output_folder):
    ...
    with open(merged_pdf_path, "wb") as fout:
        writer.write(fout)
    print(f" ")
    print(f"已经保存了第一次临时合并的发票文件 {summary_number}_第一次临时合并.pdf")
...
def append_blank_page_if_needed(merged_pdf_path, output_folder, summary_number):
    # 检查文件名是否包含特定文字
    if '1piece' in os.path.basename(merged_pdf_path):
        print(f"跳过文件 {merged_pdf_path}，因为它包含特定文字 '1piece'")
        return
...
def adjust_pages_to_a4(merged_pdf_path, output_folder, summary_number, Binding_Position):
    A4 = (595, 842)  # A4尺寸像素点
        ...
        with open(merged_pdf_path, "rb") as fin:
            ...
            for i in range(0, len(reader.pages), 2):
                ...
                if i < len(reader.pages):
                    ...
                    total_height = page1.mediabox.height + (page2.mediabox.height if page2 else 0)
                    scale = min(effective_width / page1.mediabox.width, effective_height / total_height)
    ...
    with open(output_path, 'wb') as f:
        writer.write(f)
    print(f"已经保存了第二次临时合并文件 {summary_number}_{summary_page_position}_temp4prt.pdf")    
